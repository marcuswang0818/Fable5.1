...
function check(plan) {
  ...
  for (const e of bySchool('Harvard Extension')) { if (e.credits_counted !== 4) issues.push('HES ' + e.code + ': credits must be 4'); const m = e.start.slice(5, 7); if (m >= '06' && m <= '08') issues.push('HES ' + e.code + ' starts ' + e.start + ' in summer (expensive $3,980 term) - move to Fall/January/Spring') }
